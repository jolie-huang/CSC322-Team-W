...
// Let user chose username, updated as they fill it out
router.get("/:username", async (req, res) => {
  ...
  try {
    ...
    // Ensure username users proper characters
    if (!regexUserName.test(username)) {
      return res.status(401).send("Invalid Username!");
    }

    // Don't allow username to be noprofilefound
...
router.post("/", async (req, res) => {
  ...
  // Check if the inputted email is a proper email
  if (!isEmail(email)) return res.status(401).send("Invalid Email!");

  const passwordRegex =
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{4,}$/;

  // Check if password passes requirements defined above
  ...
  try {
    ...
    // Create a new user based off given inputes
    user = new UserModel({
      name,
    ...
    // Save the profile model, and create new models which are associated to the user model
